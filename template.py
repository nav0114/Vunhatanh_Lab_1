...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def truncate(text, max_length=40):
        ...
        return text_clean if len(text_clean) <= max_length else text_clean[:max_length-3] + "..."
    ...
    header = (
        f"{'Prompt':<{w_prompt}} | "
        f"{'GPT-4o Response':<{w_resp}} | "
        f"{'Mini Response':<{w_resp}} | "
    ...
    for result in results:
        prompt = truncate(result["prompt"])
        gpt4o_response = truncate(result["gpt4o_response"])
        mini_response = truncate(result["mini_response"])
        gpt4o_latency = f"{result['gpt4o_latency']:.2f}s"
        mini_latency = f"{result['mini_latency']:.2f}s"

        row = (
            f"{prompt:<{w_prompt}} | "
            f"{gpt4o_response:<{w_resp}} | "
            f"{mini_response:<{w_resp}} | "
            f"{gpt4o_latency:<{w_lat}} | "
            f"{mini_latency:<{w_lat}}"
        )
